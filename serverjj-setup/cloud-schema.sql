...
CREATE TABLE IF NOT EXISTS chutima.web_commands (
  id text PRIMARY KEY, digest text NOT NULL, command jsonb NOT NULL,
  product_id text, created_at timestamptz NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS chutima.web_releases (
  command_id text NOT NULL REFERENCES chutima.web_commands(id),
  device_id text NOT NULL REFERENCES chutima.devices(id), product_id text NOT NULL,
  quantity integer NOT NULL CHECK(quantity>=0),
  PRIMARY KEY(command_id,device_id,product_id)
);
CREATE TABLE IF NOT EXISTS chutima.cloud_config (
