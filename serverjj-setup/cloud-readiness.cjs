...
const [ip,caFile,expectedVersion]=process.argv.slice(2);
...
function check(){return new Promise((resolve,reject)=>{
  const req=https.get({hostname:ip,port:8443,path:'/health',ca,rejectUnauthorized:true,timeout:2000},res=>{
    ...
    res.on('end',()=>{try{const value=JSON.parse(data);if(res.statusCode!==200||value.service!=='chutima-server'||value.protocol!==1||expectedVersion&&value.version!==expectedVersion)throw Error('Unexpected service');resolve();}catch(e){reject(e);}});
