...
async function start(configFile){
  ...
  await engine.transaction(async db=>{const old=(await db.query("SELECT body FROM chutima.entities WHERE collection='app' AND id='officeCapabilities'")).rows[0];if(old?.body?.version!==2)await engine.write(db,{collection:'app',key:'officeCapabilities',value:{version:2}},'office_upgrade_2');});
