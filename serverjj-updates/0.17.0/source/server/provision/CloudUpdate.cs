...
class CloudUpdate:Form {
 ...
 CloudUpdate(){Text="ชุติมา • เชื่อมเว็บ SERVERJJ 0.17.0";ClientSize=new Size(820,550);Font=new Font("Tahoma",11);StartPosition=FormStartPosition.CenterScreen;Controls.Add(log);Controls.Add(button);
  Say("อัปเดตเฉพาะบริการ ChutimaServer ที่ติดตั้งไว้แล้ว\r\nสำรองฐานข้อมูลชุติมาก่อนอัปเดต และเก็บไฟล์บริการรุ่นเดิมไว้\r\nPOS ยังบันทึกงานในเครื่องได้ระหว่างอัปเดต\r\nเพิ่มจัดซื้อ รายงาน และต้นทุนบนเว็บ โดยใช้ POS เดิมที่จับคู่ไว้");
  button.Click+=async delegate{if(working)return;working=true;button.Enabled=false;try{await Install();Say("เสร็จแล้ว: ตัวเชื่อมเว็บพร้อมทำงาน\r\nขั้นต่อไปเปิดหน้าเว็บแล้วกดอัปเดต เพื่อดูรายงานและใบสั่งซื้อ");button.Text="อัปเดตเรียบร้อยแล้ว";}catch(Exception e){Say("ยังไม่เสร็จ: "+e.Message);button.Text="ปิดหน้าต่างและส่งภาพข้อความนี้";}finally{working=false;}};
 ...
 async Task Install(){
  ...
  using(var service=new ServiceController("ChutimaServer")){
   ...
   try{
    ...
    Say("ติดตั้งระบบใบสั่งซื้อ รับสินค้า และรายงาน");replaced=true;
    ...
    Say("ตรวจบริการ HTTPS ด้วยใบรับรองเดิมของร้าน");await Run(Node,Q(Path.Combine(update,"readiness.cjs"))+" 127.0.0.1 "+Q(Path.Combine(Data,@"TLS\ca.pem"))+" 0.17.0");
