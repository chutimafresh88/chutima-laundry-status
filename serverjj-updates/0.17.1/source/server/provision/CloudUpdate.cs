...
class CloudUpdate:Form {
 ...
 CloudUpdate(){Text="ชุติมา • เชื่อมเว็บ SERVERJJ 0.17.1";ClientSize=new Size(820,550);Font=new Font("Tahoma",11);StartPosition=FormStartPosition.CenterScreen;Controls.Add(log);Controls.Add(button);
 ...
 async Task Install(){
  ...
  using(var service=new ServiceController("ChutimaServer")){
   ...
   try{
    ...
    Say("ตรวจบริการ HTTPS ด้วยใบรับรองเดิมของร้าน");await Run(Node,Q(Path.Combine(update,"readiness.cjs"))+" 127.0.0.1 "+Q(Path.Combine(Data,@"TLS\ca.pem"))+" 0.17.1");
