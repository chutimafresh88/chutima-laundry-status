...
export function createHandler({env,fetchImpl=fetch}){return async req=>{
 ...
 try{
  ...
  if(input.action==='load'){
   ...
   if(role==='employee'){const fields=['id','name','sku','barcode','packBarcode','packSize','category','unit','price','stockOnHand','lowStockAt','enabled','trackStock','showInQueue','image','updatedAt','allocations'];products=products.map(p=>Object.fromEntries(fields.map(k=>[k,p[k]??null])));}
