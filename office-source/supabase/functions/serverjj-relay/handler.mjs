// Machine access is scoped to the shop registered by an authenticated owner.
...
export function createHandler({env,fetchImpl=fetch,now=()=>new Date().toISOString()}){
  return async request=>{
    ...
    try{
      ...
      const rows=await admin('/rest/v1/inventory_central?server_id=eq.'+input.serverId+'&select=shop_id,server_id,key_hash,active,revision,generation,device_status,synced_at');
      ...
      if(input.action==='exchange'){
        ...
        const timestamp=now(),heartbeatDue=!Number.isFinite(Date.parse(central.synced_at||''))||Date.parse(timestamp)-Date.parse(central.synced_at)>60000;
        if(!sameDevices(devices,central.device_status)||(central.active&&heartbeatDue))await admin('/rest/v1/inventory_central?server_id=eq.'+input.serverId,{method:'PATCH',body:{...(!sameDevices(devices,central.device_status)?{device_status:devices}:{}),...(central.active&&heartbeatDue?{synced_at:timestamp}:{})}});
        ...
        return reply(200,{active:central.active,revision:Number(central.revision),generation:central.generation,commands,pollAfterMs:commands.length?10000:60000});
      }
