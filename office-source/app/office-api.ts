...
export type PurchaseOrder={id:string;orderNo:string;supplierId:string;supplierName:string;items:{productId:string;name:string;sku:string;quantity:number;received:number;unitCost:number}[];status:'OPEN'|'PARTIAL'|'RECEIVED'|'CANCELLED';revision:number;total:number;notes:string;expectedDate:string;createdAt:string};
...
export type Purchase = {revision?:number;refundReceivable?:number;orderNo?:string;id:string;purchaseNo:string;supplierSnapshot:{name:string};documentNo:string;total:number;paidAmount:number;balance:number;createdAt:string;items:{productId?:string;effectiveUnitCost?:number;name:string;quantity:number;unitCost:number;lineTotal:number}[]};
...
export type Snapshot = {dashboard?:{date:string;cash:Record<string,number>;queues:Record<string,number>};reportPages?:ReportPage[];reportPage?:ReportPage|null;reportVersion?:number;purchaseOrders?:PurchaseOrder[];purchaseWorkflowVersion?:number;schema:number;revision:number;products:Product[];categories:string[];suppliers:Supplier[];purchases:Purchase[];movements:Movement[]};
...
export function allowedSection(id:string){const role=currentRole();return role==='owner'||role==='manager'&&id!=='staff'||role==='reports'&&['dashboard','products','reports','history'].includes(id)||role==='stock'&&id==='products';}
